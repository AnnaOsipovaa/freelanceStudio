...
export class FreelancersService {
    ...
    static async createFreelancer(data) {
        ...
        const result = await HttpUtils.request('/freelancers', true, 'POST', data);

        if (result.redirect || result.error || !result.response || (result.response && result.response.error)) {
            returnObject.error = 'Возникла ошибка при создании фрилансера. Обратитесь в поддержку';
            if (result.redirect) {
                returnObject.redirect = result.redirect;
            }
            return returnObject;
    ...
    static async updateFreelancer(id, data) {
        ...
        const result = await HttpUtils.request('/freelancers/' + id, true, 'PUT', data);

        if (result.redirect || result.error || !result.response || (result.response && result.response.error)) {
            returnObject.error = 'Возникла ошибка при обновлении фрилансера. Обратитесь в поддержку';
            if (result.redirect) {
                returnObject.redirect = result.redirect;
            }
        }
